...
#'
#' @description
#' `day_time_of_admission` returns whether the patient was admitted during day/night time and
#' weekday/weekend.
#'
#' @details
...
#'
#' @param ipadmdad a data.table object equivalent of DRM table "ipadmdad". Table must contain two fields,
#' admission id and admission date and time
#' @param dtvar1 a character string of label equivalent to "admission date time".
...
day_time_of_admission <- function(ipadmdad,
  ...
  res[, ":="(day_of_admission_derived =
    ifelse(lubridate::wday(stringr::str_sub(dtvar1, 1, 10), label = TRUE) %in% c("Sun", "Sat"),
      "weekend", "weekday"
    ),
  time_of_admission_derived =
    ifelse(as.numeric(stringr::str_sub(dtvar1, 12, 13)) >= 8 &
      as.numeric(stringr::str_sub(dtvar1, 12, 13)) < 17,
    "daytime", "nighttime"
    ),
  dtvar1 = NULL)][]
