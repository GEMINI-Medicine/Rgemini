...
#' then truncate it to specified minimum and maximum values.
#'
...
#'
#' @description
#' The function samples from a skewed normal distribution.
#' It uses `rsn` to obtain time of day data in hours.
#' Values greater than 24 are subtracted by 24 (moved to the next day)
#' so that a real time variable is observed.
#'
...
#'
#' @description
#' The function samples from a log normal using `rlnorm`
#' to obtain time of day data in hours.
#' Values greater than 24 are subtracted by 24 (moved to the next day)
#' so that a real time variable is observed.
#'
#' @param nrow (`integer`) The number of data points to sample
...
#'
#' @description
#' This function creates a data table based on the input,
#' either based on a number of unique IDs or an existing inpatient data table.
#' It can be used to create long format data tables based on inpatient data
#' or to create a new data based on user specifications.
#'
...
#' If `cohort` is not included, then it will only have the following fields:
...
generate_id_hospital <- function(
    nid = 1000,
    n_hospitals = 10, avg_repeats = 1.5, include_prop = 1,
    cohort = NULL, by_los = FALSE, seed = NULL) {
    if (!is.null(seed)) {
        set.seed(seed)
    }
