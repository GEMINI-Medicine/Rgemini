...
rxnorm_query <- function(dbcon,
  ...
  if (grepl("_subset", pharmacy_table)) {
    query_str <- paste0(
      "WITH rc AS (",
      "  SELECT search_type, rxcui, raw_input",
      "  FROM rxnorm_cache",
      "  WHERE active = TRUE",
      "    AND rxcui IN (", search_rxcui_list, ")",
      "), ",
      "p AS (",
      "  SELECT *",
      ...
      ") ",
      "SELECT p.genc_id, rc.rxcui, rc.search_type, rc.raw_input, p.row_num",
      " FROM p INNER JOIN rc ON p.med_id_din = rc.raw_input",
      " WHERE rc.search_type = 'med_id_din'",
      " UNION ALL ",
      "SELECT p.genc_id, rc.rxcui, rc.search_type, rc.raw_input, p.row_num",
      " FROM p INNER JOIN rc ON p.med_id_ndc = rc.raw_input",
      " WHERE rc.search_type = 'med_id_ndc'",
      " UNION ALL ",
      "SELECT p.genc_id, rc.rxcui, rc.search_type, rc.raw_input, p.row_num",
      " FROM p INNER JOIN rc ON p.med_id_generic_name_raw = rc.raw_input",
      " WHERE rc.search_type = 'med_id_generic_name_raw'",
      " UNION ALL ",
      "SELECT p.genc_id, rc.rxcui, rc.search_type, rc.raw_input, p.row_num",
      " FROM p INNER JOIN rc ON p.med_id_brand_name_raw = rc.raw_input",
      " WHERE rc.search_type = 'med_id_brand_name_raw'",
      " UNION ALL ",
      "SELECT p.genc_id, rc.rxcui, rc.search_type, rc.raw_input, p.row_num",
      " FROM p INNER JOIN rc ON p.med_id_hospital_code_raw = rc.raw_input",
      " WHERE rc.search_type = 'med_id_hospital_code_raw'",
      " UNION ALL ",
      "SELECT p.genc_id, rc.rxcui, rc.search_type, rc.raw_input, p.row_num",
      " FROM p INNER JOIN rc ON p.iv_component_type = rc.raw_input",
