...
As a final step in planning our analyses, we should consider the minimum required sample size to ensure our study is feasible. The sample size we need depends on multiple factors, such as the goal of our model (inference vs. prediction), the study design (clinical trial vs. observational study), the type of model and its degrees of freedom (e.g., number of parameters), whether analyses are performed within a frequentist or Bayesian framework, the frequency of the outcome (e.g., mortality rate), and what effect sizes are considered clinically relevant (e.g., see [here](http://ndl.ethernet.edu.et/bitstream/123456789/15304/1/9103.pdf)). 

Here, we are conducting an observational study with the goal of performing inference. In this context, our sample size considerations should focus on whether we have sufficient **statistical power** - i.e., assuming that there is indeed an effect of age on mortality, we want to be sure that our analyses have a high chance of detecting this effect. Based on previous research, we would expect age to have a relatively large effect on clinical outcomes, and for simplicity, we are planning to fit a regression model with only a small number of parameters. Therefore, the required sample size should be relatively small, but how small exactly?

> **As a rule of thumb**, it has been proposed that for binary outcomes (e.g., mortality) in a logistic regression model, we should have a minimum of 10 outcome events per predictor parameter (i.e., number of predictor variables + their interactions/non-linear terms if any). As we will see below, our cohort consists of more than 8,000 encounters and typical in-hospital mortality ranges from 5-10% (i.e., we can expect ~400-800 outcome events in our cohort). Thus, even if we had a large number of model parameters, we would have sufficient statistical power. In fact, for studies using a large portion of the GEMINI cohort, sample size is typically not a limiting factor (also see [here](https://www.sciencedirect.com/science/article/pii/S1386505622000764)). However, this may not hold true if you are analyzing rare events and plan to use highly complex models. In those cases, we encourage you to explore existing sample size calculators (e.g., the [Cleveland Clinic Sample Size Calculator](https://riskcalc.org/samplesize/) or [Sample Size Calculator for Clinical Prediction Models](https://www.bmj.com/content/368/bmj.m441)), but keep in mind that these methods often rely on several assumptions and simplifications, and therefore, may only give you a rough estimate. 
...
Finally, the `data_coverage()` function returns a flag for each `genc_id` in `data` indicating whether a given encounter was discharged during a time period/from a hospital with *any* `lab` coverage (based on the min-max dates listed in the `lookup_data_coverage` table). For example, here are some (dummy) encounters from site 13 with the corresponding coverage flags for `lab`:
...
**Note: The coverage flag being `TRUE` does not necessarily mean that the corresponding `genc_id` has an entry in the `lab` table. Instead, the coverage flag only tells us that there were *some* encounters during the same discharge period and at the same hospital that had an entry in the `lab` table.** 

**Let's revisit why that's helpful to know for our analyses.**

<details>
<summary>Expand this section</summary>

Knowing overall data coverage in the GEMINI database is valuable information that can help us determine how to handle `genc_ids` that **do not** have an entry in the lab table (i.e., `genc_ids` with missing `mLAPS` - see below):

+ **If the coverage flag is `TRUE` AND the additional coverage checks you performed did not reveal any issues (e.g., sudden drops in `lab` data coverage):** We can now infer that if an encounter indeed received a lab test, it should be reflected in the `lab` table (since coverage is consistently high). Conversely, if a `genc_id` does not exist in the `lab` table (even though the coverage flag is `TRUE`), we can assume that the encounter did not receive a lab test - for example, because testing may not have been clinically indicated.

+ **If the coverage flag is `FALSE` OR if we identified any issues in the coverage checks performed above:** This means that if a `genc_id` does not exist in the `lab` table, we don't have sufficient information to determine whether a) the encounter did in fact not receive a lab test (true negative) or b) whether the encounter received a lab test but the test is not captured by GEMINI data due to coverage issues (false negative).
</details>
...
cohort_table <- cohort_creation(
  cohort = list(
    ...
    data[admitted_from_er == FALSE] # not admitted from ED
  ),
  labels = c(
  ...
  exclusion_flag = c(FALSE, FALSE, FALSE, TRUE, TRUE), # last 2 steps are EXCLUSIONS
  big.mark = ","
)
```
...
1) A `data.table` object containing all rows from the original `data` table that are included in the final cohort (N = 8,481, based on all inclusions/exclusions). We will name this table `cohort`, which will be used in all subsequent analyses below:
...
- As a reminder, you can obtain this from the `derived_variables` table, which contains a `gim` flag that is `TRUE` for all encounters that were admitted to/discharged from a GIM service.
...
Let's first take a closer look at missing values. We need to carefully consider what the reason for missing values is in order to make an informed decision about how to deal with them. In our example scenario, the only relevant variable with `NA` is `mlaps`.
...
**2. That hospital does not have lab data coverage for that encounter.** We already checked lab data coverage above and removed encounters that were discharged from hospitals/during time periods without lab data coverage. For the remaining time periods, lab coverage was consistently high (>95%).
...
+ **Other considerations**
  - Note that it's important to distinguish between "data coverage" (overall data volume for certain tables - see section above) vs. "missing values" (individual entries with `NA` values for certain table columns).** Even when data coverage is generally high, individual entries may still have missing values.
  - For time periods with high data coverage, we can sometimes make certain assumptions about missing values based on the clinical context, allowing us to perform single-value imputation (e.g., imputing all missing `mlaps` values with 0).
  - There are many cases where we cannot make these assumptions, and instead need to a) either exclude encounters with `NA` (complete-case analysis), b) use multiple imputation, c) use a missing indicator method, or d) apply modelling approaches that can otherwise handle missing values. This decision depends on various factors, such as your sample size, the type of model you are using, the goal of your model (e.g., prediction vs. inference) etc.
  - Another important consideration is whether you are dealing with variables that are **"missing at random" (MAR)** or **"missing not at random" (MNAR)**. The details of this are beyond the scope of the current vignette, but this paper by [Austin et al. (2021)](https://onlinecjc.ca/article/S0828-282X(20)31111-9/fulltext#:~:text=Missing%20data%20is%20a%20common,all%20subjects%20in%20the%20sample) provides a nice overview on this topic. 
  - **Always make sure to discuss how to handle missing values with your PI and Scientific Advisor.**
...
### Variable transformation & pre-processing
...
Additionally, we will create a categorical version of the Charlson Comorbidity Index. Note that the majority of patients have a CCI of 0-2. CCI >= 3 represents patients with several different comorbidities. For descriptive purposes (see section on Table 1 below), we will group these into a single ("high comorbidity") category:

```{r}
cohort <- cohort[, admit_charlson_derived_cat := case_when(
      admit_charlson_derived == 0 ~ "0",
      admit_charlson_derived == 1 ~ "1",
      admit_charlson_derived == 2 ~ "2",
      admit_charlson_derived >= 3 ~ "3+",
    )
]
```
...
In the previous steps, we created the cohort and performed some data preprocessing. Let's plot our cohort variables over time to check whether there are any unexpected time trends that could bias our analyses or may indicate data quality issues. This is also a good way to double check whether any of our inclusion/exclusion steps may have introduced unforeseen patterns/gaps in our data.

As an example, we'll just plot the average `mlaps` here (but you should perform similar checks for all other relevant predictor/outcome variables). We would generally expect `mlaps` to remain fairly stable over time. If you observe a sudden increase/decrease in `mLAPS`, this could indicate changes in testing practices, errors/omissions in lab mappings, or data quality issues (e.g., data coverage gaps for specific test types) that you may need to investigate further:

```{r}
Rgemini::plot_over_time(cohort, plot_var = "mlaps", func = "mean")
```

**Note:** For the purpose of data exploration and data quality checks, it can often be helpful to plot means instead of medians. Because means are more sensitive to outliers than medians (see more details in the following section), it's often easier to detect unexpected trends/outliers when plotting the mean. You can also check missingness over time by specifying `func = "na"`.

*  *  *  *

# Descriptive statistics

In the previous section, we illustrated some examples of descriptive analyses that facilitate data checks and cohort creation. We have also already checked the distribution of all variables in our cohort. So now, we can start performing our main analyses on our final cohort table (N = 8,481).

## Table 1

The first step is typically to create a table with descriptive statistics summarizing the baseline characteristics of patients in our cohort - commonly referred to as **"Table 1"**. This table usually includes demographic information (e.g., age and sex) and clinical indicators measured at admission (e.g., diagnosis codes/comorbidities, lab results, admission from ED, transfers from other hospitals/long-term care etc.). The main purpose of this table is to help others understand what type of patients were included in your study (and whether or not those patients are comparable to other studies). **You should *not* use this table to perform any kind of inference, so p-values or confidence intervals *should not* be included in Table 1. Similarly, you *should not* include outcome variables since this table is meant to reflect baseline characteristics prior to any intervention.**

`Rgemini` contains a wrapper for the `table1` package, creating a nicely formatted Table 1 while suppression any cells with n < 6 (see [here](https://gemini-medicine.github.io/Rgemini/articles/cell_suppression_and_table1.html for more information on cell suppression). Here, we will include age, sex, Charlson Comorbidity, and mLAPS in Table 1. Our continuous variables `age` and `mLAPS` are skewed (see histograms in previous section). Therefore, we will specify `continuous_fn = "median"` to obtain the median and 25th/75th percentile [Q1, Q3] because the mean [SD] is sensitive to extreme values (see [here](https://pmc.ncbi.nlm.nih.gov/articles/PMC137399/) for an introdoctory overview of different measures of central tendency and spread).

```{r}
library(Hmisc)
library(table1)

# first, let's label our variables so the names show up nicely in Table 1
label(cohort$age) <- "Age"
label(cohort$sex_female) <- "Sex = Female"
label(cohort$admit_charlson_derived_cat) <- "Charlson Comorbidity Index (at admission)"
label(cohort$mlaps) <- "mLAPS (at admission)"

# generate Table 1 with baseline characteristics
table1(~ age + sex_female + admit_charlson_derived_cat + mlaps,
  data = cohort,
...
**Note:** If your main analysis involves a group comparison (e.g., treated vs. untreated patients), Table 1 should be stratified by the relevant groups and between-group differences should be described using standardized mean differences (SMDs, see [here](https://gemini-medicine.github.io/Rgemini/articles/cell_suppression_and_table1.html#calculating-standardized-mean-differences) for more details). 

## Other descriptive analyses

You may perform other descriptive analyses at this stage if relevant. For example, you may want to check the prevalence of outcome events (in our case, we can already see the prevalence of in-hospital mortality in the summary plot created in the previous section). For time-to-event analyses, you can also plot the cumulative incidence over time.

**Importantly, any descriptive analyses you perform at this stage should not look at any associations between predictor and outcome variables. Additionally, avoid performing any type of descriptive analyses with the goal of picking relevant covariates for your model.** For example, you should never use bivariate correlations between predictors and outcomes to inform variable selection as this ignores covariance between multiple predictors and their possible interactions, and invalidates inference performed on your final model due to "double dipping" (i.e., using the same data to both generate and test your hypotheses). This is also referreed to as circular inference, which makes your analyis prone to incorrectly rejecting a true null hypothesis (inflation of type-1 error, see [here](https://pmc.ncbi.nlm.nih.gov/articles/PMC10515571/) and [here](https://pmc.ncbi.nlm.nih.gov/articles/PMC2841687/#:~:text=Abstract,a%20policy%20for%20avoiding%20circularity.)). Instead, if the goal of your analysis is to perform inference, variable selection should be informed by your conceptual framework & DAG (see section above). Although there are scenarios where data-driven variable selection may be appropriate (e.g., feature selection for predictive ML models), this should typically follow a rigorous modelling approach to avoid common pitfalls (see [here](https://onlinelibrary.wiley.com/doi/epdf/10.1111/tri.12895), [here](https://www.sciencedirect.com/science/article/abs/pii/S0895435621002146), and [here](https://www.frontiersin.org/journals/bioinformatics/articles/10.3389/fbinf.2022.927312/full) for a more detailed discussion).
