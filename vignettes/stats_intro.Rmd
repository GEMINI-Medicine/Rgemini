...
- Incl. 1: Hospitalizations with Pneumonia as most responsible diagnosis code (MRDx) in in-patient diagnosis table
...
Next, we'll check data coverage. Generally, this should be performed on the whole dataset (prior to applying inclusion/exclusion steps).
...
Next, we filter the cohort for all relevant encounters. In our example, we only want to include encounters with CCSR = "Pneumonia":
...
- For continuous variables: Does the variable look normally distributed, or is it heavily skewed?
  - For example, the distribution of length of stay is heavily right-skewed
