...
For our research question, the relevant data tables are `admdad` (containing administrative data like patient age and sex) and `lab` (containing all lab tests that are relevant for `mLAPS`). As stated in the cohort inclusions above, we only want to include encounters with high data coverage for both of these tables.

We can achieve this by running the `data_coverage` function as follows:
