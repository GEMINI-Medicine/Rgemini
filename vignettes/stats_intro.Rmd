...
**This section is meant to walk you through the decision making process that leads us to our final model. We will bulid up to the final model and build your intuition along the way.**
...
We are trying to understand the effect of age on in-hospital mortality. 

One outcome in our study is in-hospital mortality, which is binary (dead, not dead). So, we will be using a logistic regression. We use the `rms` package [3](https://cran.r-project.org/web/packages/rms/index.html), which is a fantastic resource that makes best-practices easy to implement. 
...
We see that the odds ratio is 1.044 and the 95% confidence limits are 1.037 and 1.052. Each additional year of age is associated with 1.044 increased odds of death. Our 95%CI do not cross 1, so our results are statistically significant. 

This tells us that older age is associated with more death, but it does not tell us how much death. For that, we turn to probabilities. Recall that odds are a transformation of probability. If something has a 75% probability (P=0.75), the odds that it happens are 3:1. `Odds = (probability / (1-probability))`.
...
So, now we have a single odds ratio that describes the increased odds of death per year. We are assuming that each year, the odds of death increase by the same amount as the previous year. We assume that an 18 year old turning 19 and an 89 year old turning 90 have both gained the same additional odds of death. This is a strong assumption and we need to test it! 
...
See that we now have 3 separate coefficients for age. These coefficients are essentially uninterpretable, which is a cost we pay for relaxing the nonlinearity assumption. We rely on likelihood ratio tests, predicted probabilities, and post-hoc contrasts (beyond the scope of this document) to report results from splines. 

See that the nonlinear model does not fit the data any better based on AIC and likelihood ratio test. This is supported by the plot below, which shows very similar predicted probabilities using the spline.  
...
We proceed with the linear model because describing every additional year with the same odds ratio seems to be a reasonable assumption. 
