...
<strong style="color:red;">Important notes: </strong>
  + It is crucial to **always check data coverage for all relevant tables**, regardless of whether or not you plan to exclude encounters/hospitals with gaps in data coverage.
  + In general, data coverage checks should be run on **all encounters from hospitals and time periods of interest** (e.g., data from Apr 2018 - 2020 from 20 hospitals), before applying study-specific cohort inclusions/exclusions (e.g., based on diagnosis criteria).
...
coverage <- data_coverage(
  ...
  cohort = data, # all encounters from relevant hospitals/time periods
  table = c("admdad", "lab") # relevant data tables
)
```

Briefly, the `data_coverage()` function queries the `lookup_data_coverage` table, which lists the time periods with coverage for each data table and hospital (also see Data Availability plot in the [Data Dictionary](https://geminimedicine.ca/the-gemini-database/)). The min-max dates refer to the range of **discharge dates** during which there was **at least 1 `genc_id`** with an entry in a given table. In other words, this provides a very rough check of whether GEMINI received **any** data from a given hospital during a certain time period.

By default, the function returns multiple plots:

**1) Timeline plot**

The timeline plot simply illustrates the min-max discharge dates based on the (simplified) coverage definition in the `lookup_data_coverage` table. This allows users to get a general overview of the table \* hospital combinations for which GEMINI data have **any** coverage (at least 1 `genc_id`):
...
<details>
<summary>**Let's inspect this plot. What should you look out for?**</summary>
...
Note that all GEMINI encounters by definition have an entry in the `admdad` table. However, patient volume (overall number of encounters) and the min-max dates of `admdad` coverage may vary across hospitals. Additionally, clinical variables (such as `lab` data) may only have partial data coverage, even when administrative data are fully available. This is due to challenges associated with data extraction of clinical data.

**1) Data coverage:** Certain hospitals may not have any (administrative) data during certain time periods. For example, data coverage for sites 6, 9, 10, and 12 starts later/ends earlier than for the other sites. 

**2) Patient volume:** Some hospitals have a smaller number of encounters overall (e.g., sites 6 & 15), which is important to consider for the analyses below (e.g., small community hospitals will generally contribute fewer encounters to the overall cohort than large academic hospitals).
...
to filter for encounters with `admdad` and `lab` data coverage, and plot coverage by hospital & month.

Typically, we recommend checking data coverage by hospital \* **discharge month** (because GEMINI data are pulled based on patients' discharge dates).
...
A basic sanity check is to plot the number of encounters per hospital per month. This provides information about data coverage & volume for **administrative** (DAD) data:
...
<details>
<summary>***Please review this plot carefully and think about the information you can gather from it. Then expand this section for some tips.***</summary>
...
Depending on your research question, you should consider whether hospitals with incomplete data timelines should be removed from the analyses altogether, or whether you instead want to retain any available encounters from each site. This decision comes down to whether differences in data timelines between hospitals may induce bias in your analyses. For the purpose of our example research question, we included all 20 hospitals regardless of data timelines since we do not assume that the effect of age on mortality would have changed throughout the study period from 2018-2020.

Regardless of your decision, you should always inspect data timelines and, if relevant, report differences in timelines between hospitals in your manuscript (***LINK TO DATA COVERAGE TIMELINE PLOT ONCE IT'S BEEN MERGED***).
