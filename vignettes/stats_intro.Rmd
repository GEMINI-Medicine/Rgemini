...
First, we'll check data coverage. Generally, this should be performed on the **whole dataset** within the relevant study period (2018-2020), prior to applying any additional inclusion/exclusion steps. Typically, we recommend checking data coverage by hospital \* **discharge month** (because GEMINI data are pulled based on patients' discharge dates).

Here we illustrate how you can use the `Rgemini::data_coverage()` function to explore data coverage of the relevant variables. Please review the [data coverage vignette](https://github.com/GEMINI-Medicine/Rgemini/blob/139_data_coverage/vignettes/data_coverage.Rmd) for more details. 

For our research question, the relevant data tables are `admdad` (containing administrative data like patient age and sex) and `lab` (containing all lab tests that are relevant for `mLAPS`). As stated in the cohort inclusions above, we only want to include encounters from hospitals & time periods that have data coverage for both of these tables.

We can achieve this by running the `data_coverage()` function as follows (note that this requires a connection to a database version `drm_cleandb_v3` / `H4H_template_v4` or newer):
...
By default, the function will return multiple plots:

**1) Timeline plot**

```{r}
knitr::include_graphics("figures/stats_intro_coverage1.png")
```
...
```{r}
knitr::include_graphics("figures/stats_intro_coverage2.png")
```

## `lab`

```{r}
knitr::include_graphics("figures/stats_intro_coverage3.png")
```
