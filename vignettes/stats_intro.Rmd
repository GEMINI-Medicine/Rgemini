...
## Helper functions (THESE WILL BE MOVED TO RGEMINI)
...
cohort_creation <- function(cohort, labels, exclusion_flag = NULL, show_prct = TRUE, group_var = NULL, ...) {
  ...
  if (!is.null(group_var)) {
    ...
    cohort_tab_grouped <- lapply(grouped_list, create_cohort, exclusion_flag = exclusion_flag, group_var = group_var, ...)
...
cohort_table <- cohort_creation(
  ...
  labels = c("All GEMINI encounters discharged between Apr 2018 - March 2020", 
             ...
             "Patients with a hospitalization in previous 30 days"),
...
Make sure the number of rows corresponds to what's shown in the cohort creation table above (N = 1,876)

```{r}
cohort <- data[incl1_diag == TRUE & incl3_ed == TRUE & excl1_30day == FALSE]
```
