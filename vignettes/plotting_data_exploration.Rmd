...
The plotting examples shown below are based on variables from the "admdad" table. Typically, you would query this table from the GEMINI database, but for the purpose of this vignette, we'll create some dummy data using `gemSim::dummy_admdad()`. Here, we are simulating a subset of "admdad" variables for 50,000 encounters from 12 different hospitals and with discharge dates ranging from fiscal years 2018 to 2022:
...
First, let's use the `plot_summary()` function to explore the dataset and gain a better understanding of the distributions of `admdad` variables in our dummy cohort.
...
- **Variable class:** For example, discharge disposition is of type `numeric` in "admdad". However, in reality, discharge disposition is a categorical variable so users can specify `class = "character"` to create a more appropriate plot.
...
We can also simply plot the number of rows per month * hospital by specifying `func = "n"`. This is a great way to check for potential data availability issues (e.g., time periods with 0 rows might reflect gaps in data availability). In our example here, the number of rows corresponds to the number of unique encounters in `admdad`, but depending on the table you provide as `data` input, the count of rows might reflect other variables, such as total number of pharmacy orders, lab tests etc.
