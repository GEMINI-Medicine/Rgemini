...
Finally, users can use the `plot_coverage` flag to plot the % of encounters with an entry in a given table, by hospital & discharge month. This provides more detailed insights into data coverage beyond the current coverage definition used in the `lookup_data_coverage` table (which lists all time periods with any data, even if just 1 `genc_id` had a table entry).
