...
custom_dates <- data.table(
  ...
  hospital_id = c("GRH", "GRH", "PMH", "PMH", "PMH", "MKHR", "MKHR", "MKHR"),
...
save(coverage, file = "data/data_coverage.rda")
...
transf <- dbGetQuery(dbcon, "SELECT genc_id FROM transfusion WHERE hospital_id = 'PMH';") %>%
  data.table()
cohort[, excl := hospital_id == "PMH" & genc_id %in% transf$genc_id & year(discharge_date_time) >= 2020 & as.numeric(substr(cohort$discharge_date_time, 9, 10) > 22)]
cohort <- cohort[excl == FALSE, ]
...
save(coverage, file = "data/data_coverage2.rda")
```
```{r eval = FALSE}
