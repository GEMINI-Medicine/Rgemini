...
Coverage is assessed separately for adult cohorts (adult encounters only) and paediatric cohorts (paediatric encounters only). By default, `data_coverage()` uses the adult cohort; paediatric coverage checks are relevant only for databases containing paediatric data.

Although GEMINI aims to achieve high coverage of all data elements, it is possible that some tables could not be fully extracted from certain hospitals/time periods. This is especially true for clinical variables, such as lab, transfusion, radiology, or pharmacy data.
...
lookup_data_coverage <- data.table(
  data = c("admdad", "er", "er", "pharmacy", "pharmacy", "radiology", "transfusion", "ipscu", "ipdiagnosis", "lab", "lab", "lab", "physicians"),
  hospital_num = c(1, 2, 2, 3, 3, 4, 5, 6, 7, 8, 8, 8, 9),
  hospital_id = c("GRH", "LHSCU", "LHSCU", "LHSCV", "LHSCV", "HHCO", "HHSH", "HHSJ", "HRH", "PMH", "PMH", "PMH", "MKHR"), # just adding this to ensure consistency with plots created below (based on hospital_id)
  min_date = c("2018-02-01", "2015-04-01", "2022-04-01", "2019-07-01", "2022-04-01", "2017-01-01", "2015-04-01", "", "2015-04-01", "2017-04-01", "2018-12-10", "2022-03-30", "2016-04-01"),
  max_date = c("2023-03-31", "2023-06-30", "2024-06-30", "2023-03-31", "2023-08-31", "2023-06-30", "2022-05-31", "", "2023-06-30", "2018-04-07", "2021-10-15", "2023-03-20", "2023-03-31"),
  cohort_type = c(
    "adult", "adult", "paeds", "adult", "paeds", "adult",
    "adult", "adult", "paeds", "adult", "adult", "adult", "adult"
  ),
...
+ The min/max dates in each row correspond to the start/end date of a single data coverage period by hospital\*table (listed separately for `paeds` vs. `adult` cohorts where applicable).
...
## this is not part of the vignette, it's just some internal code that can be used to produce/update the figures included in the vignette
library(DBI)
library(RPostgreSQL)
library(getPass)
library(lubridate)
...
To visualize the data timelines by hospital & table, you can set the `plot_timeline` input to `TRUE`, which will result in a plot similar to the “Data Availability” plot in the data dictionary. However, the plot returned by this function is customized to the specific tables that are relevant for your analyses (i.e., `table` input argument), and can also include custom time periods/hospitals based on an optional `cohort` input.
...
<strong style="color:red;">WARNING: </strong>
When plotting data coverage, we recommend running the `data_coverage()` function on the whole adult medicine/ICU cohort. While you may pre-filter the cohort for hospitals and time periods of interest, you **should not** apply any additional, encounter-level cohort inclusion/exclusion criteria (e.g., diagnosis codes etc.) at this point. This is to ensure that the coverage plots are representative of the overall GEMINI data holdings, and are not skewed by any project-specific cohort inclusion/exclusion steps.

<br></br>

Let's plot coverage for the transfusion table for all encounters in the medicine/ICU cohort discharged since April 2017: 
