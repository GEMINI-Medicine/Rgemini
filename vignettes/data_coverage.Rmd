...
# save to data folder
save(coverage, coverage2, coverage_cust1, coverage_cust2, file = "~/GitHub/Rgemini/data/dummy_data_coverage.rda")
```
```{r eval = FALSE}
...
For example, let's add a `hosp_label` and  `hosp_type` variable to our `admdad_subset` table:

```{r echo = FALSE}
# add hospital_label
set(admdad_subset,
  j = "hosp_label",
  value = factor(admdad_subset$hospital_num, levels = unique(sort(admdad_subset$hospital_num)),
  labels = LETTERS[1:length(unique(sort(admdad_subset$hospital_num)))])
)

# assign (random) hospital grouping variable
admdad_subset[, hospital_type := sample(
  c("Teaching", "Non-teaching"),
  prob = c(.4, .6), 1, replace = TRUE
), by = hospital_id]
```

We can now run `data_coverage` and provide these newly created variables as input to `hospital_label` and `hospital_group`. Note that different levels of `"hospital_type"` will be color-coded, and we can customize the color scheme by providing a `colors` input:
...
```{r echo = FALSE}
print(coverage_cust2 + ggtitle("Data timeline for 'lab' table with customized dates for site 4\n"))
```
