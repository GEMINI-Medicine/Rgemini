...
For example, here our cohort only contains encounters discharged since April 2017 and we are interested in the data timelines for the `admdad`, `lab`, and `ipscu` table:
...
# Plot data timelines
coverage <- data_coverage(
  db, cohort, table = c("admdad", "lab", "ipscu"),
  plot_timeline = TRUE, plot_coverage = FALSE
)
```

```{r echo = FALSE, fig.cap="Timeline plot", out.width = '80%', warning = FALSE}
load("~/GitHub/Rgemini/data/data_coverage.rda")
...
coverage <- data_coverage(
  db, cohort, table = c("transfusion"),
  plot_timeline = FALSE, plot_coverage = TRUE
)
```


In addition to the plot, this will also return a list object similar to the one above:
