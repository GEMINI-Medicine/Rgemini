...
3. **`hierarchy`**: Flag whether to apply the predefined hierarchy filters to retain only highest-priority drug information per pharmacy row.
4. **`cell_suppression`**: Flag whether to apply small cell suppression to the outputs.
...
<br>**$sme** is the frequency table to be shared with the SME for validation. To share this aggregated data file with SME, analysts must submit a file transfer request for GEMINI review. **Only approved files may be exported and shared**.
```{r, eval = F}
...
GEMINI analysts with write access to the pharmacy mapping database can proceed to upload your validated mappings using the `GEMINIpkg::add_validated_pharm_map()` following instructions in [*Using add_validated_pharm_map*](#section-db-upload) below.
