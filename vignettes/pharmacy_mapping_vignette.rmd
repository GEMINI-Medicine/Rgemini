---
title: "GEMINI Rxnorm and Pharmacy Mapping"
output: 
...
setwd("~/repos/GEMINIpkg/") # Set working directory to your GEMINIpkg repo
devtools::load_all(getwd())

## Connect to the DB that holds the Pharmacy Data Table
...
# ## Connect to the DB that holds the Pharmacy Master Mapping file
# map_db_con <- DBI::dbConnect(
#   DBI::dbDriver("PostgreSQL"),
#   dbname = "pharmacy_mapping", # Note this is a different DB than the one above
#   host = "prime.smh.gemini-hpc.ca",
#   port = 5432,
#   user = getPass::getPass("Database Username:"),
#   password = getPass::getPass("Database password: ")
...
GEMINI receives raw pharmacy tables from participating sites. The GEMINI team developed the RxNorm function (`gemini_rxnorm_query`) to identify drugs of interest easier and faster, by using the RxNorm API from the National Library of Medicine. Please refer to our publication for details at [GEMINI-RxNorm](https://academic.oup.com/jamiaopen/article/6/3/ooad062/7239332).
...
The series of functions is designed to support an analyst's workflow in identifying orders for specific drug(s) of interest from the GEMINI pharmacy data table. This vignette provides general guidelines for effectively using these functions. Analysts are encouraged to consult the function documentations for more detailed information.


## Design Overview
```{r, echo=FALSE, out.width = "100%", fig.align = "left", fig.cap="<div style='color:grey; font-size:12px; margin: 0px 0px 0px 20px'>Input-Output Flow Diagram of Three Pharmacy Functions. Items in orange denote steps requiring analyst or SME engagement. Pharmacy Master Mapping and Staging Pharmacy Mapping are database tables</div>" }
knitr::include_graphics("figures/overall_design.png", dpi = 200 )
```
...
The pharmacy mapping files (`pharmacy_master_mapping`, `staging.working_pharmacy_mapping`) are stored in the `pharmacy_mapping` database. While most processes are automated such that analysts don't interact with these mapping files directly, knowing how to query them to explore existing mappings can be handy:
```{r, eval=F}
...
# Using `gemini_rxnorm_query`

The **`gemini_rxnorm_query`** function retrieves rows from the GEMINI pharmacy data matching the user-specified drug term(s).
...
Users can clearly identify which column is found to be a match, and it only requires SME to validate one column at a time as opposed to upto 6 columns at a time with the previous function.

For users with experience with the previous version of gemini_rxnorm_query, please note the following changes:
...
2. If user input both `drug_input` and `class_input`, a union of these two independent searches will be done instead of an intersection in the previous version.

## Key parameters

There are 5 primary parameters in the function.  

1. **`drug_input`**: This should be a vector with generic drug names. While text values will be normalized (to lowercase, singularized, etc.) by subsequent functions, drug names can sometimes be tricky to singularize. Therefore, make sure to use the singular form (e.g, input "insulin", not "insulins") as this needs to comply with what is already stored in the master file. We also **recommend** to check the master file for previously mapped drugs ('rxnorm_match' & 'drug_group') and follow the same naming convention for your input if a match exists.
2. **`class_input`**: This should be a vector with ATC codes or names of drug classes. For example for insulin, user can input ATC class "A10AB" or class name "Insulins and analogues for injection, fast-acting".
...
4. **`genc_ids`**: Optional vector of `genc_ids` to search against. If no input is provided, all `genc_ids` will be included. When `return_unmatched` is TRUE the function returns an error if no `genc_ids` input is provided because returning the `unmatched_rows` for all pharmacy entries will result in memory issues.
5. **`return_drug_list`**: Optional parameter allowing users to obtain a list of drugs (rxcui, drug_name) when running `gemini_rxnorm_query` with `class_input`. Default is FALSE.

**`NOTE`**:

- There are 3 different ways to input your drug identifiers into this function:
    1.  Input a vector of `drug_input`: This is the **recommended way** as it does not introduce complexity. We recommand using `drug_input` solely, instead of `class_input` or a combination of both `drug_input` and `class_input`.
    2.  Input a vector of `class_input`: The function will prompt user to confirm a list of ATC classes and the drugs associated with the ATC classes. Users are indirectly querying based on drug names and heavily rely on the drug name to ATC relationship from rxnorm.
        +  If `class_input` is used, it is best to provide ATC codes at the level between 2 to 4 according to the Anatomical Therapeutic Chemical Classification system by the World Health Organization. See [WHO ATC](https://www.who.int/tools/atc-ddd-toolkit/atc-classification) for details. ATC codes at level 1 (e.g. "J: Antiinfective for systemic use") lacks specificity. ATC codes at level 5 (e.g. "J01CA04: amoxicillin") are too detailed that they cannot be searched as a drug class in `class_input`
        +  When `class_input` is used, the function internally converts the drug classes into a list of drug names and then queries based on the drug names. We recommand users to turn on `return_drug_list` when using `class_input`. This will output a list of drug names associating with the `class_input`, allowing users to confirm the list with an SME. 
    3.  Input both `drug_input` and `class_input`: Previously an **intersection** of these two independent searches (one based on `drug_input` and one based on `class_input`) will be used. In this version, a **union** of these two searches are used instead.

## Insulin example

```{r, eval = F}
rxnorm_res <- gemini_rxnorm_query(
  db_con = db_con, # DB contains the Pharmacy Data
  drug_input = c("insulin"),
  genc_ids = example_cohort
)

head(rxnorm_res %>%
  arrange(genc_id, row_num)) # some digits of encounter-level identifiers are hidden to protect privacy.
```
...
rxnorm_res %>%
  ...
  mutate(genc_id = replace_last(genc_id, 3), row_num = paste0("...", substr(row_num, 6, 7))) %>%
  head()
```

```{r, echo=FALSE, out.width = "80%", fig.align = "center"}
knitr::include_graphics("figures/Insulin_simple_example.PNG", dpi = 100)
```
...
1. Search by the individual drug name or the individual active component name. Example 1.
...
-  User should input c("trimethoprim", "sulfamethoxazole") into `drug_input`, instead of combining the two drugs into one customized entry like "trimethoprim-sulfamethoxazole" or "trimethoprim/sulfamethoxazole".
...
rxnorm_res_ex1 <- gemini_rxnorm_query(
  db_con = db_con,
  drug_input = c("trimethoprim", "sulfamethoxazole"),
  genc_ids = example_cohort
)

rxnorm_res_ex1[grepl("\\|", rxnorm_match)] %>%
  arrange(genc_id, row_num) %>%
  head() # Last digits of encounter-level identifiers are hidden to protect privacy.
```

```{r, eval=F, echo=F}
#code to create figure below
rxnorm_res_ex1[grepl('\\|', rxnorm_match)] %>% mutate(genc_id = replace_last(genc_id, 3), row_num = replace_last(row_num, 3)) %>% arrange(genc_id, row_num) %>% head()
```

```{r, echo=FALSE, out.width = "80%", fig.align = "center"}
knitr::include_graphics("figures/trim_sulf.PNG", dpi = 200)
```
...
-  User should input c("complera") in `drug_input`.

-  User can perform a second search on the individual ingrediants (see Example 1) to potentially find additional hits. For drugs like this one, which are available under one specific brand name, searching by individual components can be less sensitive than directly searching the brand name.

```{r, eval = F}
# Example 2 Complera
rxnorm_res_ex2 <- gemini_rxnorm_query(
  db_con = db_con,
  drug_input = c("complera"),
  genc_ids = example_cohort
)

tail(rxnorm_res_ex2) #Last digits of encounter-level identifiers are hidden to protect privacy.
```

```{r, eval=F, echo=F}
#code to create figure below
rxnorm_res_ex2 %>% mutate(genc_id = replace_last(genc_id, 3), row_num = replace_last(row_num, 3)) %>% tail()
```

```{r, echo=FALSE, out.width = "80%", fig.align = "center"}
knitr::include_graphics("figures/complera.png", dpi = 200)
```


## Important notes
1. If a user input more than one drugs in `drug_input` or enter a class `class_input`, under `rxnorm_match` a pipe "|" may appear to separate more than one drugs which indicates that each entry (cell) from the pharmacy table is found to link to these drugs. 

2. Only drugs entered into `drug_input` or drugs under `class_input` selected by users will appear under `rxnorm_match` output (even if the pharmacy entry may contain an additional drug the user didn't search for).

3. Rxnorm is a US-based tool. Although this is highly uncommon, drugs that are only prescribed in Canada (and not in the US) cannot be found by RxNorm. In addition, the current `gemini_rxnorm_query` tool does not support searches based on Canadian brand names. Please use the equivalent American brand name or generic name instead.

4. If `gemini_rxnorm_query` returns a warning saying "drug could not be found", please check the [RxNav website](https://mor.nlm.nih.gov/RxNav/) as sometimes there may be alternative searchable drug names based on the suggestion from the search bar. For example, Paxlovid returns the same warning, but when entered into RxNav, an auto-completion in the search bar shows "paxlovid 150 mg /100 mg dose pack". In this case, user should input "paxlovid 150 mg /100 mg dose pack" into `drug_name`.

5. This function requires database version `drm_cleandb_v3_1_0` and newer. It is backward **incompatible** with database version before that, and `rxnorm_top_score` in the output has been deprecated.
...
RxNorm is not a perfect tool and may generate false-positive matches. A typical workflow we recommend is consulting a SME to validate the results. This could be physician, pharmacist, or anybody with knowledge of medication. They can ensure that the pharmacy records identified by RxNorm are indeed accurate.

**`prepare_pharm_for_validation`** was developed to streamline this validation process. It helps analysts to process the RxNorm results into standardized frequency tables for validation by the SME and subsequent analyses.

3 key advantages of using this function:

- It compares the RxNorm results against previously validated mappings in the Pharmacy Master Mapping file, flagging matches that had already been validated before by other projects. This allows SMEs to focus primarily on new matches, streamlining the validation process.
...
1. **`rxnorm_res`**: Object returned by `gemini_rxnorm_query`.
2. **`hierarchy`**: Flag whether to apply the predefined hierarchy filters to retain only highest-priority drug information per pharmacy row.
3. **`cell_suppression`**: Flag whether to apply small cell suppression to the outputs.
4. **`outpath`**: Optionally provide a file path where the results will be saved. File export is only permitted when `cell_suppression=TRUE`.
...
### Pre SME validation
We have already run RxNorm search for insulin in the above section by `rxnorm_res <- gemini_rxnorm_query(...)`, we can now pass 'rxnorm_res' to the second function `prepare_pharm_for_validation` to process the search results into frequency tables for SME validation:
...
Upon running, the user will be prompted to provide their username and password to establish a connection to the database hosting the Pharmacy Master Mapping file.
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/prepare_pharm_run.png", dpi = 200)
```
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/freqtab_sme.png", dpi = 200)
```
...
- **`count`**: number of unique genc_ids associating with each row. With cell_suppression = T, small counts less than 6 encounters are suppressed to "<6".
- **`search_type`, `raw_input`, `rxnorm_match`**: distinct rxnorm results.
- **`drug_group`**: general classification of the drug in `rxnorm_match` found in the Pharmacy Master Mapping file. `drug_group = NA` for drugs that have not been previously mapped.
...
- Note that text values in `search_type`, `raw_input`, `rxnorm_match` and `drug_group` have been **standardized**. The function converts them to lowercase, singularizes certain fiels (`rxnorm_match` and `drug_group` only), and handles special charaters & leading/trailing whitespaces. See in-package function `normalize_text` for details.
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/freqtab_ana.png", dpi = 200)
```

- Same table as prep_res$sme, with one additional column: `pharm_row_num`. 
- **`pharm_row_num` is a LIST storing `row_num`** - a unique identifier for each row of the pharmacy table. Analysts will need these values to retrieve encounter-level data from the pharmacy table after SME-validation. See [*Guidelines for post-validation*](#section-post-valid). Since `pharm_row_num` is encounter-level data, this table is restricted to internal use only. **Do Not Share Outside The Internal Environment**. For illustration purposes of the Vignette, the last digits of `pharm_row_num` are hidden.


### SME validation

When the user specifies a folder path to `outpath`, a file named `pharmacy_mapping_for_SME.xlsx` is exported to the corresponding folder. This file corresponds to the frequency table in `prep_res$sme` in the user's GlobalEnv. It is intended for sharing with the SME for validation.

Analyst should clearly explain to the SME how to use this table to conduct validation. Below are the key guidelines:
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/sme_valid_example_pre.png", dpi = 200)
```
</div>
  ...
  6. In cases where *drug_group* is missing: If applicable to the project, the SME should complete the *drug_group* column manually. Otherwise, the analyst can reach out to GEMINI's internal SME to fill it out later, which is **required** when adding the validated mappings to the database (see [*Using add_validated_pharm_map*](#section-db-upload)).
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/sme_valid_example.png", dpi = 200)
```
...
<hr style="border-top: 0.5px solid #ccc; margin: 20px 0;">
Recall the goal of the Insluin study - determine the proportion of encounters had an order for 'insulin' during their hospital stay. 
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/rds_object.png", dpi = 200)
```
</div>
...
sme_valid_clean <- read.csv(file.path(getwd(), "Vignettes/figures/pharmacy_mapping_for_SME_validated.csv"))

head(sme_valid_clean)
...
insulin_pharm_row_num[1:5] #Last digits of encounter-level identifiers are hidden to protect privacy.
# [1] "547810.." "547810.." "547812.." "547866.." "547866.."
```
</div>

<p style="padding-left: 35px; margin-top:20px;">**STEP 3.** Identify encounters (`genc_id`) corresponding to the pharmacy orders.</p>

  <div style="padding-left: 40px; font-size:13px">**$pharm_row_num** values (e.g. 547810.., 547871..) correspond to **row_num** of the pharmacy table, which is the unique identifier for *each pharmacy row*. Therefore, analysts can use them to trace each entry back to the original pharmacy table to identify genc_ids with **at least one insulin order throughout hospitalization**:</div>
...
insulin_pharm_row_num <- data.table(row_num=insulin_pharm_row_num) #ensure it's data.frame

dbWritetable(db_con, c("pg_temp", "temp_row_num"), insulin_pharm_row_num,
  temporary = T, row.names = F) # write the row_num into a temp table
...
insulin_cohort[1:5] # Last digits of encounter-level identifiers are hidden to protect privacy.
# [1] 28937... 28938... 28938... 28938... 28940...
...
<br><div style="padding-left: 40px; font-size:13px">In the pharmacy table, one encounter can have multiple pharmacy orders. **row_num** is, therefore, also useful for *tracing order-level information*. For example, instead of identifying encounters with at least 1 insulin prescription, the study is now interested in identifying encounters who had **at least one insulin prescribed during the first 24 hours of inpatient admission**. The analyst can use row number along with order date-time to refine the cohort:</div>

<div style="padding-left: 40px">
```{r, eval=F}
# Pull pharm rows containing validated drugs
dbWritetable(db_con, c("pg_temp", "temp_row_num"), insulin_pharm_row_num,
  temporary = T, row.names = F) # write the row_num into a temptable
...
adm_query <- paste0(
  ...
  paste(example_cohort, collapse = ", "), ");"#if cohort is large, send the list of genc_id as a temp table instead
) 
...
insulin_24hr_cohort[1:5]
# [1] 28937... 28938... 28938... 28940... 28942...
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/prepare_pharm_nohier_run.png", dpi = 200)
```
...
rxnorm_res %>%
  filter(row_num == 1695237) %>%
  mutate(
    genc_id = "fake_id 1", # for purpose of the vignette, hide last digits of genc_id to protect privacy
    row_num = paste0("...", substr(row_num, 6,7)) # for purpose of the vignette, hide last digits of row_num to protect privacy
  )
```

```{r, echo=FALSE, out.width = "80%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/nohier_example.png", dpi = 200)
```

```
- For this order, the columns `med_id_generic_name_raw`, `med_id_brand_name_raw`, and `med_id_din` each contain drug information, all of which are matched to "insulin" by RxNorm. When `hierarchy=T`, only the row with "med_id_generic_name_raw = insulin regular 100 unit/mL vial (1,000 unit/10 mL)" is included in the result frequency table, while other rows are dropped, ensuring a one-to-one match. When `hierarchy=F` all rows will be included, therefore, the redundant information "med_id_din = 00586714" and "med_id_brand_name_raw = Novolin Toronto ; Humulin R" are returned and contribute to the frequency count.
```

### Unmatched results

Recall that with **return_unmatched=TRUE** in the previous `rxnorm_res <- gemini_rxnorm_query(...)` function, the returned object contains 2 items - matched and unmatched results. When `rxnorm_res` contains unmatched result, **`prepare_pharm_for_validation` will automatically detect its presence, and process them along with the matched result to facilitate SME validation.**

```{r, eval=F}
rxnorm_res_unmatch <- gemini_rxnorm_query(
  db_con = db_con,
  drug_input = c("insulin"),
  genc_ids = example_cohort,
  return_unmatched = T)
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/prepare_pharm_unmatch.png", dpi = 200)
```
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/filter_pharm_unmatch.png", dpi = 200)
```
</details>
...
iv_route <- freqtab_route %>% filter(row_id %in% c(1, 4, 6, 7, 9)) %>% pull(route)
...
iv_insulin <- dbGetQuery(db_con, pharm_query) %>%
  filter(tolower(route) %in% iv_route) 
```
...
Once the SME-validated mappings are finalized, they should be integrated into GEMINI's Pharmacy Master Mapping file. **Analysts are expected to document and perform the integration**:
...
2. Document the mapping information on the [Solo mapping tracker](https://app.smartsheet.com/sheets/hgGCpcp9PhQ25RfW9pPpQjm5jJ9mxwmRmhQRQHP1?view=grid)

3. With the final mapping file fulfilling the above criteria, analyst can proceed to run the function [add_validated_pharm_map()](#section-db-upload), which automates databse upload processes for analysts.
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/masterfile_collection_truepos.png", dpi = 200)
```
...
Below is the expected formatted file, after the analyst filters mappings to true positive rows:
```{r, echo=FALSE, out.width = "80%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/masterfile_collection_truepos_clean.png", dpi = 200)
```

<br>**False negatives** are drug entries missed by RxNorm and found by examining unmatched rows. In the mock example below, the blue-highlighted row is an entry that RxNorm missed but was identified as insulin by the SME.

```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/masterfile_collection_falseneg.png", dpi = 200)
```
...
Below is the expected *final* formatted file for database upload (containing both the true positives and the false negative entry):
```{r, echo=FALSE, out.width = "80%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/masterfile_collection_falseneg_clean.png", dpi = 200)
```
...
**`add_validated_pharm_map`** was developed to accurately and efficiently append new validated pharmacy mappings to the existing collection of pharmacy mappings stored in the Pharmacy Mapping database. 
...
For new pharmacy mapping records that have passed the checks, the function will **appends them to the Staging Pharmacy Mapping table (`staging.working_pharmacy_mapping` in the database)**, along with the user's database `user_name`, `project_id`, and a timestamp (`last_updated`). The added user information allows tracking of records in case a reversion is necessary.
...
3. **`Project Title`**: the Abbreviated Title per GEMINI Project Tracker of the research project. If the mapping is not associated with a project on the tracker, the user should provide a brief name that describes the mapping activity, e.g. 'reports_sedhypno_indicator'.
...
5. **`CSV Output Path`**: if the user chooses to export unmapped records without `drug_group`, they must then enter path for the csv file which these records will export to.
...
Below is an example of preprocessing the SME validated pharmacy mapping file so that it can run smoothly in `add_validated_pharm_map`:
```{r, eval=F, echo=T}
# read the SME validated mapping file
pharmacy_mapping <- fread(file.path(getwd(), "Vignettes/figures/pharmacy_mapping_for_SME_validated.csv"))
...
fwrite(pharmacy_mapping, file.path(getwd(), "Vignettes/figures/pharmacy_mapping_for_SME_validated_updated.csv"))
```

Run the function on the cleaned file:

```{r, eval=FALSE, echo=T}
add_validated_pharm_map(file.path(getwd(), "Vignettes/figures/pharmacy_mapping_for_SME_validated_updated.csv"))
```
...
```{r, echo=FALSE, out.width = "80%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/add_validated_pharm_map_figures/figure_2_invalid_records_SME_not_TRUE_quit_error_message.png", dpi = 200)
```

```{r, echo=FALSE, out.width = "80%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/add_validated_pharm_map_figures/figure_1_invalid_records_SME_not_TRUE_table.png", dpi = 200)
```
</details>
...
```{r, echo=FALSE, out.width = "80%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/add_validated_pharm_map_figures/figure_3_user_name_prompt.png", dpi = 200)
```
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/add_validated_pharm_map_figures/figure_4_duplicate_mapping_entries_message_and_user_acknowledgement.png", dpi = 200)
```
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/add_validated_pharm_map_figures/figure_5_viewer_of_rows_to_be_appended.png", dpi = 200)
```
...
```{r, echo=FALSE, out.width = "80%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/add_validated_pharm_map_figures/figure_6_final_confirm_prompt.png", dpi = 200)
```
...
pharmacy_mapping_2 <- fread(file.path(getwd(), "Vignettes/figures/pharmacy_mapping_for_SME_validated_updated.csv"))
...
fwrite(pharmacy_mapping_2, file.path(getwd(), "Vignettes/figures/pharmacy_mapping_for_SME_validated_updated_no_drug_group.csv"))
```

```{r, eval=FALSE, echo=T}
### run the function on the newly updated file
add_validated_pharm_map(file.path(getwd(), "Vignettes/figures/pharmacy_mapping_for_SME_validated_updated_no_drug_group.csv"))
```
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/add_validated_pharm_map_figures/figure_8_no_drug_group_export_prompt_yes.png", dpi = 200)
```
...
```{r, echo=FALSE, out.width = "100%", fig.align = "center", fig.cap="" }
knitr::include_graphics("figures/add_validated_pharm_map_figures/figure_9_no_drug_group_export_prompt_no.png", dpi = 200)
```
...
# 1   liuweih med_id_generic_name_raw          acenocoumarol acenocoumarol oral anticoagulant GEMINI-Low-Risk_DVT 2024-11-01 09:30:06
# 2   liuweih med_id_generic_name_raw               apixaban      apixaban oral anticoagulant GEMINI-Low-Risk_DVT 2024-11-01 09:30:06
# 3   liuweih med_id_generic_name_raw    apixaban 2.5 mg tab      apixaban oral anticoagulant GEMINI-Low-Risk_DVT 2024-11-01 09:30:06
# 4   liuweih med_id_generic_name_raw apixaban 2.5 mg tablet      apixaban oral anticoagulant GEMINI-Low-Risk_DVT 2024-11-01 09:30:06
# 5   liuweih med_id_generic_name_raw      apixaban 5 mg tab      apixaban oral anticoagulant GEMINI-Low-Risk_DVT 2024-11-01 09:30:06
# 6   liuweih med_id_generic_name_raw   apixaban 5 mg tablet      apixaban oral anticoagulant GEMINI-Low-Risk_DVT 2024-11-01 09:30:06
