...
For users with experience with the previous version of rxnorm_query, please note the following changes:
...
5. **`return_drug_list`**: Optional parameter allowing users to obtain a list of drugs (rxcui, drug_name) when running `rxnorm_query` with `class_input`. Default is FALSE.
...
1. **`rxnorm_res`**: Object returned by `rxnorm_query`.
...
Recall that with **return_unmatched=TRUE** in the previous `rxnorm_res <- rxnorm_query(...)` function, the returned object contains 2 items - matched and unmatched results. When `rxnorm_res` contains unmatched result, **`prepare_pharm_for_validation` will automatically detect its presence, and process them along with the matched result to facilitate SME validation.**
