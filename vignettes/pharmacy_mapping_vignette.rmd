...
Please note that this workflow is only supported for the following database versions:
- GEMINI staff: `drm_cleandb_v3_1_0` or newer 
- H4H users: `gemini_h4h_template_v5_0_0` or newer
...
db_con <- DBI::dbConnect(
  ...
  dbname = "db_name", # For H4H users: template DB
  host = "domain_name.ca",
...
# H4H users additionally need to set their data cut schema as follows:
...
staging <- dbGetQuery(map_db_con, "SELECT * FROM staging.working_pharmacy_mapping;") # GEMINI staff only
```
</details>
...
```
  ...
  4. In cases where `rxnorm_match` contains multiple matches separated by a pipe (e.g. “cefuroxime | vancomycin”) and not all matches are correct, the SME should still input TRUE in *SME_confirm*, and then specify the correct match(es) in *SME_comment*.
  
  5. While analysts may modify the table or add extra columns to meet specific project needs, *SME_confirm* should always be included. And analysts should communicate clearly to SME the distinction between rxnorm validation and cohort inclusion.
  
  6. *times_validated* allows SME to prioritize verifying entries that haven't been validated yet (times_validated = 0). However, SME should also review previously validated entries and flag any errors they find.

  7. Missing *drug_group*: the SME can choose to fill out the *drug_group* column if it is relevant for the project. For GEMINI analysts, *drug_group* field must be completed for database upload; please reach out to GEMINI's internal SME to fill it out before adding the validated mappings to the database (see [*Using add_validated_pharm_map*](#section-db-upload)).

  8. No rows should be deleted from the table.
...
## Alternative for Step 1, analysts may also use regex on all columns in the unmatched output to check if RxNorm/historical mappings have missed any entries that might correspond to the drug of interest
...
# then merge any relevant entries identified in the unmatched results with the unmatched RxNorm output to identify original pharmacy rows and their corresponding row_num
...
1. Analyst should format the SME-validated mapping file (.xlsx) according to the following requirements:
