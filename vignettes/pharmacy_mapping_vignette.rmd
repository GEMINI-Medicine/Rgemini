...
<br>Now that the SME has confirmed and reject some rxnorm matches, how do analysts go back and identify entries from the `pharmacy data table` corresponding to the confirmed matches? How can one link entries between `$sme`, `$analyst` and the `pharmacy data table`?
...
**HPC4Health users** are encouraged to contribute their mappings for inclusion in the our database, enabling reuser in future work. To prepare your mapping for database upload, please follow the requirements below.
...
1. Analyst should format the mapping file according to the following requirements:
    ...
    + In some cases, `rxnorm_match` may contains multiple matches separated by a pipe (e.g. "cefuroxime | vancomycin"). If the SME confirms only one or some of the matches, the analyst **should edit `rxnorm_match` to retain only the confirmed entries**.
...
2. **For HPC4Health users**:
    + Notify GEMINI team when a new validated mapping is ready for upload and provide location of the formatted file.
...
The expected *final* formatted file for database upload (containing both the true positives and the false negative entry):
```{r, echo=FALSE, out.width = "80%", fig.align = "center", fig.cap="" }
