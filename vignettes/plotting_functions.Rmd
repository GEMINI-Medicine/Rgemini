...
### Returning aggregated data {#return_data}
...
# Plotting theme & colors {#plot_theme}

## Plot theme {#plot_theme}

All plots shown above apply a common theme specified in the function `Rgemini::plot_theme()`. If you like the appearance of the plots shown above, you can apply this theme to any other `ggplots` you create. `Rgemini` also contains a few color palettes you can add with `gemini_colors()` (see [below](#gemini_colors)). For example:
...
`plot_theme()` accepts inputs specifying the font size/type or aspect ratio. Any additional changes can be made with `ggplot2::theme()`, for example:

```{r fig.width = 3.5, fig.height = 3.5}
my_plot + 
  plot_theme(base_size = 16, base_family = "mono", show_grid = TRUE, aspect_ratio = 0.75) +
  theme(plot.title = element_text(face = "italic")) + # additional components not controlled by plot_theme inputs
  ggtitle("Plot with GEMINI theme 2")
```

## Color palettes {#gemini_colors}

To view all color palettes currently included in `Rgemini`, you can run:
...
Hex values for an individual palette can be obtained by running `gemini_colors()` with a numeric input (e.g., `gemini_colors(5)` for "Bubblegum Burst") or character input that matches the starting character(s) of a given palette (e.g., `gemini_colors("b")` for "Bubblegum Burst").
