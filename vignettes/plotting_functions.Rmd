...
- `"median"`
- `"%"` or `"perc"` to plot % of encounters in a specified category. Requires additional input `plot_cat` specifying which category/factor level to plot (by default: for logical variables, will plot rows where `plot_var` category = `TRUE`). Note: For any character/logical/factor variables, the function will automatically infer `func = "%"`. 
- `"count"` or `"n"` to plot the number of rows/encounters


For example, to plot the number of encounters, simply specify `func = "n"` (no `plot_var` input needs to be provided in this case):
...
For example, to plot the percentage of female encounters:
