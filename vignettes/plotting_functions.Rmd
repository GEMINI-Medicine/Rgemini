...
For example, we could plot `age` separately by `gender` by specifying `gender` as a `facet_var`. Since there are not many encounters with gender = `"O"`, we may want to apply cell suppression to any data points with n < 6 by specifying `min_n = 6`:
...
```{r}
knitr::kable(head(aggr_data[[1]]))
```


The 2nd list entry corresponds to the results aggregated by grouping variable (if any), such as `hospital_type` in this example, and time interval:
```{r}
knitr::kable(head(aggr_data[[2]]))
```
