...
- `show_overall`: Flag indicating whether or not to plot thick line showing overall value across all sites
...
If a `facet_var` is provided, users can provide additional input arguments that are passed to `lemon::facet_rep_wrap()` (wrapper for `ggplot2::facet_wrap()`). For example, this allows users to control the number of rows (`nrow`) and columns (`ncol`) in facet plots and whether the scales should be "fixed" (default) or "free". For example, to plot the number of encounters per quarter at each site in a 2 * 6 facet plot where the y-axis can vary across plots:
...
plot_hosp_time(
  ...
  ylimits = c(100, 300) # will fix y-limits in all plots to this range
)
```
