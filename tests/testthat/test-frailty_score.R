...
testthat::test_that("Correct score calculated for frailty conditions and sublevels codes", {
  set.seed(2)
  cohort_dum <- data.frame(genc_id=c(1), age=c(70))
  ...
  res <- frailty_score(cohort_dum, ipdiag_dum, erdiag_dum, componentwise = F)
  testthat::expect_equal(res$frailty_score_derived, 2)

  res <- frailty_score(cohort_dum, ipdiag_dum, erdiag_dum, componentwise = T)
...
testthat::test_that("Setting erdiag to NULL removes erdiagosis codes with a warning message", {
  ...
  testthat::expect_warning( res <- frailty_score(cohort_dum, ipdiag_dum, NULL, componentwise = F) )
  testthat::expect_equal(res$frailty_score_derived, 1)

  testthat::expect_error(frailty_score(cohort_dum, ipdiag_dum, componentwise = F)) #expect error when no value is passed
})

testthat::test_that("Encounters not qualify for frailty assessment are excluded from results", {
  set.seed(2)
  cohort_dum <- data.frame(genc_id=c(1, 2, 3, 4), age=c(89, 64, 65, 70)) #id=3 below age cutoff, id=4 doesn't have diagnosis data
  ...
  testthat::expect_warning(res <- frailty_score(cohort_dum, ipdiag_dum, erdiag_dum, componentwise = F))
