testthat::test_that("Correct score calculated for frailty conditions and sublevels codes", {
  ...
  ipdiag_dum <- gemSim::dummy_diag(nid = 1, nrow = 5, ipdiagnosis = T, pattern = "I509$|M12") # 2 unique frailty conditions in map and their sub-codes
  erdiag_dum <- gemSim::dummy_diag(nid = 1, nrow = 4, ipdiagnosis = F, pattern = "I50$|M083") # not frailty conditions
  ...
  testthat::expect_equal(sort(res$diagnosis_code), c("I509", "M120", "M123", "M128"))
})

testthat::test_that("Setting erdiag to NULL removes erdiagosis codes with a warning message", {
  set.seed(2)
  cohort_dum <- data.table(genc_id = c(1), age = c(70))
  ipdiag_dum <- gemSim::dummy_diag(nid = 1, nrow = 3, ipdiagnosis = T, pattern = "M12") # frailty conditions
  erdiag_dum <- gemSim::dummy_diag(nid = 1, nrow = 3, ipdiagnosis = F, pattern = "C20$|E209$") # frailty conditions
...
testthat::test_that("Encounters not qualify for frailty assessment are excluded from results", {
  ...
  ipdiag_dum <- gemSim::dummy_diag(nid = 3, nrow = 10, ipdiagnosis = T, pattern = "C20$|R460$") # frailty conditions
  erdiag_dum <- gemSim::dummy_diag(nid = 3, nrow = 3, ipdiagnosis = F, pattern = "C20$|E209$") # frailty conditions
...
testthat::test_that("Encounters no frailty condition are returned with a frailty score of 0", {
  ...
  ipdiag_dum <- gemSim::dummy_diag(nid = 1, nrow = 10, ipdiagnosis = T, pattern = "A05$") # no frailty condition
