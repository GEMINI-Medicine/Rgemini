# Function to create dummy lab data needed for unit tests
dummy_lab <- function(id, ntest, omop, value, unit, mintime){
  dat<- data.table(
    genc_id = rep(id, ntest),
    test_type_mapped_omop = omop,
    result_value = value,
    result_unit = rep(unit, ntest),
    collection_date_time =  format(as.POSIXct(mintime, tz = "UTC") + sample(0:(24*60*60 - 1), size=ntest, replace = TRUE),"%Y-%m-%d %H:%M")
  )
  return(dat)
}

# Function to create dummy admdad data needed for unit tests
