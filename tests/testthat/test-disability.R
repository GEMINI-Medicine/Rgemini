test_that("global and component-wise outputs align", {
  ...
  # create dummy data
  set.seed(1)
  ipdiagnosis <- gemSim::dummy_diag(nid = 100)
  erdiagnosis <- gemSim::dummy_diag(nid = 80, ipdiagnosis = FALSE)
  ...
  check1_no_cat <- disability(
    cohort,
    ipdiag = ipdiagnosis, erdiag = erdiagnosis, component_wise = FALSE
  )
  check1_cat <- disability(
    cohort,
    ipdiag = ipdiagnosis, erdiag = erdiagnosis, component_wise = TRUE
  )

  ## Check genc_ids with disability == TRUE
  expect_equal(
    c(sum(check1_no_cat$disability, na.rm = TRUE), length(unique(check1_cat$genc_id))), c(56, 56)
  )

  ## All genc_ids where global disability flag = TRUE should exist in component-wise output (and v.v.)
  expect_true(
    all(check1_no_cat[disability == TRUE]$genc_id %in% check1_cat$genc_id == TRUE)
  )
  expect_true(
    all(check1_cat$genc_id %in% check1_no_cat[disability == TRUE]$genc_id == TRUE)
  )
  ...
  expect_equal(
    unique(check1_no_cat[genc_id == 888 | genc_id == 999, disability]), NA
  )
})
...
test_that("returned with disability = TRUE", {
  ...
  # check subcategories
  expect_equal(
    unique(check2[grepl("^F840", diagnosis_code), disability_category]), "Developmental Disabilities"
  )
  expect_equal(
    unique(check2[grepl("^S07", diagnosis_code), disability_category]), "Physical disability - Permanent Injuries"
  )
  expect_equal(
    unique(check2[grepl("^M05", diagnosis_code), disability_category]), "Physical disability - Musculoskeletal disorders"
  )
  expect_equal(
    unique(check2[grepl("^Q66", diagnosis_code), disability_category]), "Physical disability - Congenital Anomalies"
  )
  expect_equal(
    unique(check2[grepl("^H90", diagnosis_code), disability_category]), "Sensory disabilities - Hearing impairments"
  )
  expect_equal(
    unique(check2[grepl("^H30", diagnosis_code), disability_category]), "Sensory disabilities - Vision impairments"
  )
  expect_equal(
    unique(check2[grepl("^G25", diagnosis_code), disability_category]), "Physical disability - Neurological disorders"
  )
})
...
test_that("returned with disability = FALSE", {
  ...
  ipdiag <- gemSim::dummy_diag(
    nid = 5, ipdiagnosis = TRUE,
    pattern = "^I4|^A0|^B92|^K8|^Q2|^G65|^H92|^F83|^Z994|^H92|^E12|^Z88"
  )
