...
test_data <- data.frame(
  ...
  age = c(400, 50, 60, 87, NA, 67, 95, 63, 79, 58, 91, 45),
  gender = c("F", "M", "F", "M", "F", "M", "F", "O", "F", "M", NA, "F"),
  discharge_date_time = c(
    "2021-01-02 11:00", "2022-02-03 07:00", "2022-01-04 08:00", "2022-08-04 08:00", "2021-01-02 19:00",
...
test_that("cell count (func = 'n') returns expected values", {
  ...
  expect_equal(res$data_aggr_overall$n, c(3, 0, 0, 0, 0, 0, 0)) # missing entries are returned as 0 for counts
  ...
  expect_equal(res$data_aggr$n, c(7, 1, 1, 1, 1, 1, 1))
...
test_that("percentage (func = `prct`) of variable is returned as expected", {
  ...
  expect_equal(round(res$data_aggr$mean_age, 1), c(42.0, 65.0, 71.7, 65.7, 95.0, 58.0))
  expect_equal(round(res$data_aggr_overall$mean_age, 1), c(64.2, 70.0))
})
